...
def get_bucket(aliconfig):
    auth = oss2.Auth(aliconfig.access_key_id, aliconfig.access_key_secret)
    bucket = oss2.Bucket(auth, aliconfig.host, aliconfig.bucket)
    return bucket
...
def get_token(aliconfig, upload_dir):
    ...
    h = hmac.new(aliconfig.access_key_secret.encode(
        'utf-8'), policy_encode, sha)
