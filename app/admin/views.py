...
def blogs():
    if request.method == "POST":
        ...
        limit = data["limit"]
        pagination = (
            Article.query.filter_by(article_status="submited")
        ...
        return jsonify(
            {
                ...
                "data": [article.admin_to_json() for article in articles],
            }
        )
...
def recycle_bin():
    if request.method == "POST":
        ...
        limit = data["limit"]
        pagination = (
            Article.query.filter_by(article_status="recycling")
            .order_by(Article.finish_time.desc())
            .paginate(page, per_page=limit, error_out=False)
        )
...
def draft():
    if request.method == "POST":
        ...
        limit = data["limit"]
        pagination = (
            Article.query.filter_by(article_status="draft")
            .order_by(Article.finish_time.desc())
            .paginate(page, per_page=limit, error_out=False)
        )
...
def delete_blog():
    ...
    if request.method == "POST":
        ...
        if data["operation"] == "recycle":
            ...
            db.session.commit()
            return jsonify({"status": 200, "message": "放入回收站成功"})
            ...
            db.session.commit()
            return jsonify({"status": 200, "message": "恢复成功"})
    ...
    if request.method == "POST":
        ...
        try:
            ...
            image = ArticleImage(
                ...
                full_path=full_path,
                size=size,
                image_url=prefix_url + full_path,
            )
            db.session.add(image)
            return jsonify({"status": "200", "url": prefix_url + full_path})
        return jsonify(
            {"status": "400", "message": "could not upload this image"}
        )
...
def write_blog():
    ...
    if request.method == "POST":
        ...
        if images:
            ...
            db.session.commit()
        return jsonify({"status": "200", "id": article.id})
    ...
    return render_template(
        "admin/blog_mg/write_blog.html", form=form, types=types
    )
...
def update_blog(id):
    ...
    types = ArticleType.query.all()
    return render_template(
        "admin/blog_mg/update_blog.html",
        form=form,
        types=types,
        article=article,
    )
...
def users():
    if request.method == "POST":
        data = json.loads(request.get_data().decode("utf-8"))
        page = data["page"]
        pagination = User.query.order_by(User.join_time.desc()).paginate(
        ...
        for id in data["objects"]:
            ...
            if comment is not None:
                try:
                    ...
                    print(e)
                    db.session.rollback()
                    return jsonify({"status": 400, "message": "删除失败"})
                else:
                    db.session.commit()
            return jsonify({"status": 200, "message": "删除成功"})
    ...
    if request.method == "POST":
        ...
        if comment is not None:
            try:
                ...
                db.session.rollback()
                return jsonify({"status": 400, "message": "回复失败"})
            else:
                db.session.commit()
        return jsonify({"status": 200, "message": "回复成功"})
...
def admin():
    ...
    return render_template(
        "admin/main.html", blog_count=blog_count, user_count=user_count
    )
